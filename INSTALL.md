...
| tree-sitter Java grammar | downloaded by the `tree-sitter-language-pack` **Python package itself** from its GitHub releases (not apt, not `pip install`) — `install.sh` triggers this once so the download happens during install rather than on first scan | `~/.cache/tree-sitter-language-pack/` (per-user, not inside the repo) | AST-based structural findings (weak crypto, SQL injection, insecure logging, etc.) and AST-based hardcoded-secret-field detection |

**The tree-sitter Java grammar download is worth calling out specifically.** Unlike every other dependency in this table, it isn't installed by `install.sh` running `apt`/`pip` — `pip install -r requirements.txt` only installs the `tree-sitter-language-pack` *package*; that package then downloads the actual compiled grammar (~22MB) from GitHub Releases the first time `get_parser("java")` runs, and caches it per-OS-user under `~/.cache/tree-sitter-language-pack/`. `install.sh` triggers that download explicitly right after the pip step specifically so it happens once, during install, on a network the operator is already watching — rather than silently the first time an APK is scanned. If that download can't complete (blocked/filtered access to `github.com`/`objects.githubusercontent.com` — common on VPN-routed or firewalled pentest networks), the app **still starts and still runs**: static analysis continues via manifest/certificate/regex/TruffleHog detection, it just skips the AST-based findings until the grammar successfully downloads. See the troubleshooting table below.
...
| `CERBERUS_JADX_TIMEOUT` | `600` (seconds) | Max time allowed for a single jadx decompile; increase further if you're scanning unusually large APKs and still seeing timeout failures |
...
| Log shows `jadx decompile failed ... timed out after N seconds`, scan falls back to the weaker legacy scan | The APK is large/complex enough that a full decompile legitimately takes longer than `CERBERUS_JADX_TIMEOUT`, often worsened by low available RAM causing JVM GC pressure | Raise `CERBERUS_JADX_TIMEOUT` (default `600`); if it's still not enough, free up RAM on the host or scan on a machine with more cores/memory |
| `./run.sh` fails immediately with `tree_sitter_language_pack.DownloadError: ... timeout: global` (crash on every start), or `run.sh` warns "tree-sitter Java grammar not cached" | `tree-sitter-language-pack` couldn't download the compiled Java grammar (~22MB) from GitHub Releases — blocked/filtered/proxied access to `github.com`/`objects.githubusercontent.com`. Especially common on Kali/Parrot boxes routed through a VPN, a client-network proxy, or an engagement network with restricted egress. **If you hit the crash specifically, update to the current version of this app first** — older builds called this at server startup with no fallback, so a single network blip took the whole server down; current builds catch this and just skip AST-based findings instead. | Confirm outbound HTTPS to `github.com` works (`curl -sI https://github.com`), then re-run `./install.sh` (it pre-warms this cache) or manually run `backend/venv/bin/python3 -c "from tree_sitter_language_pack import get_parser; get_parser('java')"`. If the network is permanently restricted, this can be skipped entirely — the server runs fine without it, just without AST-based structural findings (manifest, certificate, and regex/TruffleHog secret detection are unaffected) |
